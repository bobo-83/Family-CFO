...
def seed_showcase_data(engine: Engine) -> bool:  # noqa: PLR0915 - one linear script
    ...
    checking = repository.create_account(engine, hh, "Everyday Checking (showcase)", "checking", "USD")
    ...
    txn(checking.id, 45, 2_198_700, "Online Transfer", "Online Transfer / Payment: Credit")
    ...
    for month in range(6):
        txn(card.id, 4 + 30 * month, -1_549, "NETFLIX.COM *1122")            # bill @ stale 12.99
        ...
        txn(card.id, 2 + 30 * month, -13_900, "Blue Fin Swim Academy")          # suggestion
