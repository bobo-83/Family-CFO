...
MODEL_CATALOG: tuple[ModelInfo, ...] = (
    ...
    # Qwen3-VL: unlike Qwen2.5-VL, the chat template RENDERS TOOLS (verified
    # 2026-07-10 against the repos' chat_template files) — real one-model-for-
    # both options. Tool-call quality in our agent loop: validate on apply.
    ModelInfo(
        id="Qwen/Qwen3-VL-30B-A3B-Instruct-FP8",
        label="Qwen3-VL 30B-A3B FP8 — chat + photos in ONE model, fast",
        role="both",
        parameters_b=30,
        est_memory_gb=33,
        est_disk_gb=31,
        ...
        gated=False,
        notes="MoE with ~3B active params: fast decode on bandwidth-limited boxes. Vision AND a tools-rendering template — a genuine all-in-one.",
    ),
    ModelInfo(
        id="Qwen/Qwen3-VL-32B-Instruct-FP8",
        label="Qwen3-VL 32B FP8 — strongest all-in-one for ~120GB boxes",
        role="both",
        parameters_b=32,
        est_memory_gb=35,
        est_disk_gb=33,
        ...
        supports_vision=True,
        gated=False,
        notes="Dense 32B, vision + tools in one container. Slower than the 30B-A3B MoE but stronger per token.",
    ),
    ModelInfo(
        id="Qwen/Qwen3-VL-8B-Instruct",
        label="Qwen3-VL 8B — light all-in-one",
        role="both",
        parameters_b=8,
        est_memory_gb=17,
        est_disk_gb=16,
    ...
    ModelInfo(
        ...
        supports_vision=True,
        gated=False,
        notes="Cannot call the financial tools (the VL chat template omits them) — best as the photo model; as the chat model, answers fall back to deterministic snapshots.",
    ),
    ModelInfo(
        id="Qwen/Qwen2.5-VL-72B-Instruct",
        label="Qwen2.5-VL 72B — strongest photos (no tool calling)",
        role="both",
        parameters_b=72,
        est_memory_gb=145,
        est_disk_gb=140,
        ...
        supports_vision=True,
        gated=False,
        notes="Cannot call the financial tools — best as the photo model. Needs very large memory (multi-GPU or 160GB+ unified).",
    ),
    ModelInfo(
        id="Qwen/Qwen2.5-VL-72B-Instruct-AWQ",
        label="Qwen2.5-VL 72B AWQ — biggest photo model for ~120GB boxes",
        role="both",
        parameters_b=72,
        est_memory_gb=45,
        est_disk_gb=41,
        ...
        supports_vision=True,
        gated=False,
        notes="4-bit quantized: 72B-class vision in ~45 GB — the strongest PHOTO model on GB10-class hardware. Cannot call the financial tools, so pair it with a tool-calling chat model.",
    ),
    ModelInfo(
