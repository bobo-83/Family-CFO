...
def test_normalize_merchant_strips_store_numbers_and_punctuation() -> None:
    assert normalize_merchant("NETFLIX.COM *1122") == "netflix com"
...
async def test_payment_tracked_as_loan_account_is_not_suggested(demo_client, demo_token) -> None:
    ...
    headers = _headers(demo_token)
    await _seed_recurring_charges(demo_client, demo_token, "DEPT OF EDUCATION", amount_minor=8_153)
    ...
    # Model it as a student-loan account with the same $81.53 monthly payment.
    created = await demo_client.post(
        "/api/v1/accounts",
        headers=headers,
        json={
            ...
            "type": "student_loan",
            "currency": "USD",
            "minimum_payment": {"amount_minor": 8_153, "currency": "USD"},
        },
    )
    assert created.status_code == 201
