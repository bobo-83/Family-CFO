...
@pytest.mark.anyio
async def test_get_ai_runtime_config_returns_disabled_default_when_unset(demo_client, demo_token) -> None:
...
async def test_viewer_cannot_update_ai_runtime_config(demo_client, demo_viewer_token) -> None:
    response = await demo_client.put(
        ...
        json={"provider": "vllm", "base_url": "http://vllm.local:8000", "model": "x", "enabled": True},
    )
...
async def test_put_upserts_existing_config(demo_client, demo_token) -> None:
    first = await demo_client.put(
        "/api/v1/ai/runtime",
        headers={"Authorization": f"Bearer {demo_token}"},
        json={"provider": "vllm", "base_url": "http://vllm.local:8000", "model": "a", "enabled": True},
    )
    second = await demo_client.put(
        "/api/v1/ai/runtime",
        headers={"Authorization": f"Bearer {demo_token}"},
        json={"provider": "ollama", "base_url": "http://ollama.local:11434", "model": "b", "enabled": False},
    )
