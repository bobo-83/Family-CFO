...
- [x] Implement + tests (matched-pair suppression; text-rule suppression; include-verdict override; reject hides + restore returns; coverage warning present for short history, absent for full-window history) + contract (additive fields) + generated client + deploy + live verify + commit. Verified: 359 api + 107 web tests pass. Live on the real household: 7 inflows hidden as internal transfers — all six bank-labeled "Internal Transfer Credit Savings" deposits AND a \$[redacted] "Online Transfer" unmasked by the matched-pair rule (its amount left a sibling account the same week); 10 genuine external deposits remain offered, including the recurring ~\$[redacted] biweekly transfers that look like a paycheck; coverage warning correctly names the Apr 15, 2026 history start.
...
- [x] Implement + tests + contract (additive `goal_target`) + client + deploy + verify + commit. Verified: 384 api + 108 web tests pass (the demo fixture's own \$18k EF goal surfaced during testing and three older tests were updated to the goal-aware semantics; multiple goals resolve to the LARGEST target). Live on the reporting household: \$[redacted] against the \$90,000 goal now reads **getting_started** with an \$[redacted] gap (was "fully funded"); the demo household legitimately stays fully funded (\$67k vs its \$18k goal); goal types render as labels.
...
User report (2026-07-13, from the phone): asked what was available to spend, the advisor answered "$[redacted] liquid, minus $[redacted] emergency fund, leaves **$[redacted] truly available for discretionary spending**" — taking no account of bills about to fall due or debt owed.
...
- [x] Verified LIVE on the box against the real household (2026-07-13). It reproduced the reported bug exactly — liquid $[redacted] − emergency fund $[redacted] = the $[redacted] the app had shown — and corrected it to **$[redacted]** by subtracting the bills due. End-to-end through the real vLLM the advisor called `get_safe_to_spend` (and only it), quoted the corrected figure, and stated the household's $[redacted] of debt unprompted.
- [x] Report the debt, don't just warn about it. The household carries $[redacted] across three credit cards and NOT ONE has a minimum payment recorded, so nothing was subtracted for debt and the advisor said nothing about it. `total_debt` is now an output — deliberately NOT subtracted (a balance is not due this month) but always reported, because "$6,765 to spend" said beside a silent $[redacted] of card debt is a true sentence that misleads. The unmodeled-debt warning names the amount and says how to fix it; the prompt REQUIRES the model to state the debt in the same answer.
