...
- [x] Implement + tests (matched-pair suppression; text-rule suppression; include-verdict override; reject hides + restore returns; coverage warning present for short history, absent for full-window history) + contract (additive fields) + generated client + deploy + live verify + commit. Verified: 359 api + 107 web tests pass. Live on the real household: the bank-labeled internal-transfer deposits hidden, plus a large "Online Transfer" unmasked by the matched-pair rule (its amount left a sibling account the same week); the genuine external deposits remain offered, including recurring biweekly transfers that look like a paycheck; coverage warning correctly names the history start.
...
User request (2026-07-11): (1) a likely paycheck arriving biweekly cannot auto-detect because the bank labels every transfer "Online Transfer" — one merchant key mixing paycheck-sized deposits with much larger one-offs fails the amount-consistency gate; sub-cluster by amount within a merchant group. (2) Make the tax estimate accurate for where the family lives: ask for their location. Privacy scope: only the STATE is collected (a full street address has no use in this app).
...
### M72 Follow-up: Browser E2E in CI (Issue #148)

- [x] Spec gate: extend the existing `deploy-e2e` job and `scripts/e2e-deploy-test.sh` orchestration rather than adding a second stack path. CI installs the lockfile dependencies with Node 22 and only Playwright Chromium plus its Linux libraries, then opts into all `apps/web/e2e/*.spec.ts` tests against the same isolated, TLS-served, demo-seeded, no-GPU deterministic stack while it remains alive. CI uses one worker and zero retries so shared-fixture mutations cannot race and a partial attempt cannot be hidden by retrying. A browser failure fails the job; missing Node, npm, or Chromium is an error, never a skip. The E2E Compose override replaces the production `/mnt` host bind with a disposable project volume so tests cannot touch off-box backup data and the path works on Docker Desktop. The script's EXIT trap preserves teardown on success and failure, capturing Compose status/logs before a failed stack is removed. The workflow uploads the HTML report, retained failure traces/screenshots, and stack diagnostics when not cancelled, retains the existing disk cleanup, reports disk usage around the large install/build phases, and has a 35-minute hard timeout. The web README documents the same opt-in script path for local verification. Advisor access: non-goal — this CI increment adds no family-visible data domain.
- [x] Implement, run the browser-enabled deployment path locally, update documentation, and open the focused PR. Verified on macOS against the isolated deterministic stack: production image build, migrations, demo seed, login/chat/TLS smoke checks, and all 6 Playwright tests passed (one worker, 2.9 s browser run), then no project containers or volumes remained. Deliberately observed browser failures returned a nonzero script status, retained the HTML report plus traces/screenshots, captured Compose status/logs before teardown, and still removed every project container and volume.
...
User report (2026-07-12): the Overview claimed the emergency fund was "fully funded" with a small balance saved against a much larger emergency-fund goal — the status was computed ONLY as months-of-bills coverage, which a sparse bills list makes absurdly optimistic; and the top-goal card rendered the raw enum (`emergency_fund`) instead of a label.
...
- [x] Implement + tests + contract (additive `goal_target`) + client + deploy + verify + commit. Verified: 384 api + 108 web tests pass (the demo fixture's own \$18k EF goal surfaced during testing and three older tests were updated to the goal-aware semantics; multiple goals resolve to the LARGEST target). Live: a small balance against a much larger declared goal now reads **getting_started** with the gap named (was "fully funded"); the demo household legitimately stays fully funded (\$67k vs its \$18k goal); goal types render as labels.
...
- [x] **M85-ios: iOS data-file attach UI** (2026-07-13, on macOS/Xcode 26). Delivered exactly as scoped below: `ChatAttachment` gained a `Kind` (`.visual(mediaType)` / `.dataFile`) so a non-image attachment is representable at all; `LiveAdvisorAPI.sendMessage` branches onto `data_file_base64` + `data_file_name`; `AttachmentTranscoder.dataFile` enforces the 7 MB cap without transcoding; the chat picker gained a "Spreadsheet or CSV" importer over CSV/TSV/plain-text/XLSX/XLSM. 5 new tests, including one that asserts against the JSON actually put on the wire (a captured-transport test) that a CSV lands on the data-file fields and NOT on the image fields — the exact confusion this two-field shape invites. Original scoping:
  - `Networking/AdvisorAPI.swift` — `ChatAttachment.mediaType` is typed `Components.Schemas.ChatRequest.ImageMediaTypePayload`, so the model can only express an image/PDF today. Widen it to an enum (e.g. `case visual(ImageMediaTypePayload)` / `case dataFile`), and branch when building the request: visual → `imageBase64` + `imageMediaType` (unchanged), data file → `dataFileBase64` + `dataFileName` (the server sniffs format from the filename, so the name is load-bearing, not cosmetic).
  - `Chat/AttachmentTranscoder.swift` — add `dataFile(from:displayName:)`: no transcoding, just the existing `maxRawBytes` (7 MB) cap and a `dataFileTooLarge` error alongside `pdfTooLarge`.
  - `Chat/ChatView.swift:52` — widen `.fileImporter(allowedContentTypes: [.pdf])` to also accept the formats the server actually parses: `.commaSeparatedText`, `.tabSeparatedText`, `.plainText`, and XLSX/XLSM (`UTType(filenameExtension:)` — there is no stdlib UTType for them). Reuse the existing filename-chip presentation.
  - Tests: transcoder cap/rejection, and an `AdvisorAPI` test asserting a CSV lands on `dataFileBase64`/`dataFileName` and NOT on `imageBase64` (the bug this shape invites).
  - Done when: attaching a CSV on the phone returns an answer citing real headers/sums from the file, matching what web chat already does through the same endpoint.
...
- [x] **M87-ios: iOS natural-voice playback** (2026-07-13, on macOS/Xcode 26). The phone now answers in the on-box Kokoro voice. `KokoroSpeechSynthesizer` fetches `POST /voice/tts` sentence by sentence, synthesizing the NEXT sentence while the current one plays, so time-to-first-audio is one sentence rather than the whole answer; `FallbackSpeechSynthesizer` composes it over the M86 system voice so the optional service's 503 degrades structurally rather than as a branch in the view model; `VoiceSessionViewModel` is untouched and still sees only `SpeechSynthesizing`. Two details worth keeping: it deliberately does not set an `AVAudioSession` category (`VoiceAudioSession` owns `.playAndRecord`; stomping on it would kill the mic that barge-in needs), and a mid-answer failure throws `RemainingSpeech` carrying only the *unspoken* text so the fallback resumes instead of replaying sentences the user already heard. 9 new tests incl. interrupt-does-not-resurrect-the-answer. Original scoping: `Voice/SpeechSynthesizerService.swift:4-6` notes the Kokoro stream "will slot in behind the same protocol", `VoiceSessionViewModel` takes a `SpeechSynthesizing` by injection (`:31`, `:41`) and drives it through `speak(_:)`/`stopSpeaking()` only. So M86's system voice stays as the fallback and nothing in the session state machine changes.
  - New `Voice/KokoroSpeechSynthesizer.swift: SpeechSynthesizing` — calls the generated `synthesizeSpeech`, which returns `.ok(.audioMpeg(HTTPBody))`. Play the MP3 as it arrives (the point of M87a is time-to-first-audio, not batch): pull from the `HTTPBody` async sequence into an `AVAudioPlayer`/`AVPlayer` feed rather than buffering the whole response.
  - Fallback, not replacement: the operation's generated `503` case (`serviceUnavailable`, returned whenever `FAMILY_CFO_TTS_URL` is empty or the `tts` container is down) — and any transport error — must degrade to the existing `SpeechSynthesizerService`. Cleanest shape is a small composing `SpeechSynthesizing` that tries Kokoro and falls back, so the fallback is structural and not a branch inside the view model.
  - `stopSpeaking()` must cancel the in-flight request *and* playback — this is what makes tap-to-interrupt keep working, and it is the prerequisite for the acoustic barge-in M86 deferred here.
  - Swap the injected synthesizer at the composition root that builds `VoiceSessionViewModel`.
...
- [x] M91b bulk-by-merchant (2026-07-13, user request): assigning a transaction to a category files EVERY other uncategorized transaction with the same merchant at once — categorize "Starbucks" once and all Starbucks are done. Match is on what the row shows (merchant, or description when there's no merchant), case-insensitive; transactions with no name aren't bulked. Each is set server-side, a refused one comes back in place, and only the ones that took are recorded — undo reverts the whole batch to its original order. The undo bar names the count ("Set 5 Starbucks to Dining") so a bulk assignment isn't a silent surprise. Plus a one-tap starter category set (14 defaults) with custom creation kept. 11 new tests. Original M91a:
- [x] M91a inline category creation (2026-07-13, user hit the dead end on their phone): the empty-categories state was a wall — 200 transactions, nothing to assign, and category management was web-only. Added a **New category…** option (a toolbar +, and inside the per-transaction picker) using the existing `createCategory` endpoint; created mid-categorize, it immediately assigns the transaction that prompted it. Case-insensitive dedupe avoids a needless 409; a real 409 surfaces a readable "already exists" message. A recorded, deliberate deviation from the "category management is web-only" line (08-mobile-spec) — full management still lives on the dashboard. 4 new tests; 113 iOS green.
...
- [x] Spec gate: (a) home-screen widget: net worth + EF status (WidgetKit, reads cached last-known values; no background polling of the box beyond iOS budget). (b) App Intents/Siri: "Ask my CFO …" routes the spoken question into the chat pipeline. (c) Local notifications for upcoming bills computed on device from synced bill due dates — no push infrastructure, nothing leaves the LAN/tailnet.
- [x] (b) Siri / App Intents — implemented (2026-07-13). `AskCFOIntent` + `FamilyCFOShortcuts` route a spoken question through the SAME grounded chat pipeline (ADR 0018) and return the answer as dialog, markdown stripped; logic lifted into a testable `AskCFO` enum. Runs when the app is closed (bootstraps its own `AppModel`, fails clearly if unpaired).
- [x] (c) Local bill notifications — implemented (2026-07-13). Reminders computed ON THE PHONE from the household context's `upcoming_bills` (already fetched by the Overview — no extra poll), fired the morning before each due date; idempotent refresh that cancels reminders for bills that dropped off the list and never touches foreign notifications.
- [x] (a) Widget — code + data layer implemented (2026-07-13); Xcode target wiring deferred to the UI. The app writes an `OverviewSnapshot` (net worth, EF status/months, timestamp) to a shared App Group on every Overview load and reloads the widget timeline; the widget (`FamilyCFOWidget/`, small + medium) reads ONLY that cache, never polling the box, and shows the value "as of" its capture time. The pure `OverviewSnapshot`/`Store` (no API-client dependency) is ready to compile into the widget target. **The remaining step — adding the Widget Extension target + the App Group capability — is deliberately NOT scripted:** adding an App Group entitlement is a signing/provisioning change that can break the *device* build (and thus the OTA install pipeline) if hand-edited, and it can't be verified on the remote phone from here. Documented as a one-time Xcode-UI operation in `apps/ios/README.md`.
- [x] Tests + commit. (2026-07-13: 12 new — reminder timing, stale/foreign-notification safety, intent routing + empty-question guard, snapshot round-trip/formatting. 109 iOS green.)
- [ ] Human step: add the Widget Extension target + `group.com.familycfo.ios` App Group in Xcode (see `apps/ios/README.md`), then verify the widget on the physical iPhone.

## M93: Safe-to-Spend (advisor ignored bills and debt)

User report (2026-07-13, from the phone): asked what was available to spend, the advisor answered liquid minus emergency fund as "truly available for discretionary spending" — taking no account of bills about to fall due or debt owed.
...
- [x] Verified LIVE on the box (2026-07-13). It reproduced the reported bug exactly — liquid minus emergency fund matched the figure the app had shown — and corrected it by subtracting the bills due. End-to-end through the real vLLM the advisor called `get_safe_to_spend` (and only it), quoted the corrected figure, and stated the household's card debt unprompted.
- [x] Report the debt, don't just warn about it. The household carried five-figure card debt across several cards and NOT ONE had a minimum payment recorded, so nothing was subtracted for debt and the advisor said nothing about it. `total_debt` is now an output — deliberately NOT subtracted (a balance is not due this month) but always reported, because a safe-to-spend figure said beside a silent card debt is a true sentence that misleads. The unmodeled-debt warning names the amount and says how to fix it; the prompt REQUIRES the model to state the debt in the same answer.
...
- [x] Add a synchronous I/O-free retention module with immutable policy,
...
- [x] Unit-test every cutoff and overlap, ties, cadence/sparse independence,
...
- [x] Dependency: WI-1. Owner boundary: pure module/tests only; no persistence or
      filesystem/SMB behavior.

### WI-3 — Migrate to box-global persisted settings

- [x] Add the next-head migration (expected `0093`) for the `backup_settings`
...
- [x] Implement transaction-safe bootstrap: deterministic legacy household
...
- [x] Dependencies: WI-1 and WI-2 policy types.

### WI-4 — Make inventory, capacity, and mutation lifecycle safe

- [x] Add strict stable local/SMB inventories, path containment, timestamp
...
- [x] Add same-destination partial writes/atomic promotion and cleanup; the
...
- [x] Add FastAPI schemas/services/routes for global config and
...
- [x] Edit authoritative OpenAPI before clients. At the pinned baseline move
...
- [x] Verify runtime OpenAPI parity, auth matrix, password semantics, aliases,
...
- [x] Dependencies: WI-3 and WI-4.

### WI-6 — Add web configuration and disclosure

- [x] Regenerate the web client; add status service/data flow; implement
...
- [x] Render accessible localized target/candidate, probe, capacity, coverage,
...
- [x] Verify generated drift/compatibility, unit tests, build, i18n extraction and
...
- [x] On macOS/Xcode, regenerate Swift; extend `BackupAPI`, draft/view model,
...
- [x] Add focused view-model/view tests and run Swift generation/compatibility
...
- [x] Update backup/API/database/deployment documentation, `.env.example`, and
...
- [x] Declare and provision PostgreSQL 17 in the backend workflow; add
      environment-gated real migration/bootstrap and advisory-lock tests with
      loud required-mode semantics and the required workflow path filters.
- [x] Record completed targeted/full API, web, OpenAPI, fixture, client
      compatibility, version, generated-client, and Swift/Xcode checks from the
      WI-2 through WI-7 implementation commits.
- [ ] Confirm the required PostgreSQL 17 migration/bootstrap and advisory-lock
      tests execute successfully in CI.
- [ ] Perform and record the supported-NAS manual stat-volume/quota/upload/
      rename/list/delete check through user-controlled credentials.
- [ ] Deploy API and worker first, verify live config/status and real local/SMB
      backup paths plus a guarded restore, then deploy web and TestFlight/OTA
      separately.
- [ ] Close #116 only after both clients truthfully show observed recovery and
      degraded paths and a new archive restores through the guarded workflow.
