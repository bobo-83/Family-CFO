...
def test_build_extract_archive_round_trip() -> None:
    ...
    restored_dump, restored_tar, manifest = extract_archive(archive)
...
def test_manifest_round_trip() -> None:
    manifest = {"app_version": "0.127.0", "schema_revision": "0073_backup_versioning"}

    archive = build_archive(b"dump", b"docs", manifest=manifest)
    restored_dump, restored_tar, restored_manifest = extract_archive(archive)

    assert restored_dump == b"dump"
    assert restored_tar == b"docs"
